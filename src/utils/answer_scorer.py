...
def main(prediction_dataset_path: str, predicted_answer_field: str, ground_truth_field: str) -> None:
    ...
    for idx, row in tqdm(enumerate(prediction_dataset)):
        predictions.append({'prediction_text': row[predicted_answer_field], 'id': row[idx]})
        references.append({'answers': {'answer_start': [0], 'text': [row[ground_truth_field]]}, 'id': row[idx]})
