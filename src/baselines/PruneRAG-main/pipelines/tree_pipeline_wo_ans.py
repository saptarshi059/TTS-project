...
class Generator:
    def __init__(self, config: Config):
        ...
        if 'qwen' in self.config.model_name:
            ...
            self.subquery_template = get_subqueries_qwen3_8b_wo_ans()
            ...
            self.subquery_template = get_subqueries_llama3_8b_wo_ans()
    ...
    def _generate_subqueries(self, nodes: List[ContextTreeNode]) -> List[List[str]]:
        ...
        for i, subqueries_text in enumerate(outputs_list):
            ...
            combined_json_pattern = re.compile(
                r'((' + decomposition_re + r')|(' + entity_re + r'))',
                re.DOTALL
            )
    ...
    def generate(self, **sampling_params) -> List[str]:
        ...
        strategy.save_results(result_path, "tree_wo_ans", self.config.split, self.total_time, self.start_time, self.retrieval_num, apply_backoff=False)
        


        ##记录检索到的文档信息
        t =self.start_time.strftime("%m%d.%H:%M")
        self.save_list_of_list_of_lists_to_jsonl(retrieval_info, result_path  + "/tree_wo_ans." + f"{self.config.split}." + f"{t}.context.jsonl")
        ...
        for output, root in zip(outputs, root_nodes):
            ...
            try:
                t=self.start_time.strftime("%m%d.%H:%M")
                log_path= self.config.log_dir +f"/{self.config.model_name}"+f"/{self.config.dataset_name}"+f"/{t}._tree_wo_ans_query_tree.jsonl"
        ...
        # 记录子查询生成结果
        jsonl_path = self.config.log_dir + f"/{self.config.model_name}" + f"/{self.config.dataset_name}"+f"/outputs"+ f"/{t}_tree_wo_ans_subqueries_outputs.jsonl"
