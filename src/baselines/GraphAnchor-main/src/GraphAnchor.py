...
def init_reasoning_plus(query, refs, previous_reasoning):
    return call_llm_template("init_reasoning_plus",
                             {"query": query, "refs": refs, "previous_reasoning": previous_reasoning})
...
def step_reasoning_plus(query, refs, previous_reasoning):
    return call_llm_template("step_reasoning_plus",
                             {"query": query, "refs": refs, "previous_reasoning": previous_reasoning})
    ...
    try:
        while ("insufficient" in status.lower()) and step < args.max_step:
            ...
            try:
                ...
                return {"id": doc_id, "question": query, "ground_truth": ground_truth, "skip": True,
                        "skip_reason": f"retrieve_failed: {e}"}
...
if __name__ == "__main__":
    ...
    if args.method == "GraphAnchor":
        passage_predictions = [data["passage_output"] for data in all_result if
                               data.get("passage_output") not in [None, "", []]]
        graph_predictions = [data["graph_output"] for data in all_result if
                             data.get("graph_output") not in [None, "", []]]
        passage_graph_answers = [data["ground_truth"] for data in all_result if
                                 data.get("passage_output") not in [None, "", []] and data.get("graph_output") not in [
                                     None, "", []]]
